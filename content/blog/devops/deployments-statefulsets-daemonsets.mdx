...
description: "The workload types boil down to one question: are your replicas interchangeable, or do they have identity? Get that right and the Deployment/StatefulSet choice makes itself — including why Relay's Postgres is (controversially) not a StatefulSet."
...
Next up, we stop theorizing: [getting Kubernetes running locally](/blog/devops/4-kubernetes-running-locally), where the first Deployment of `relay-api` goes onto a real cluster and immediately greets me with `ImagePullBackOff`.
