...
Next up in the build: [multi-replica scaling and load balancing](/blog/devops/6-multi-replica-scaling), where the Service finally gets more than one pod to spread traffic across — and where keeping an order-status cache in process memory stops being a cute shortcut and starts being a bug.
